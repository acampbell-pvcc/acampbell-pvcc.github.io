...
# Program Purpose: This program reads a comma-delinated data file into
#   a LIST of smaller lists. The data for each student is appended to
#   a list with six data fields:
...
def read_data_file():
    datafile = open(student_file, "r") #The "r" indiates this is a READ file
...
def calculate_grades():
    ...
    for i in range(len(stu)):
        hw = int(stu[i][2]) #Last name in pos. 0, first name in pos. 1, so hw is in pos.2
        midex = int(stu[i][3]) #Midterm exam is in position 3
        (stu[i][4]) = (stu[i][4])[:-1] #Remove the \n from the final exam string(end of input line)
...
def create_report_file():
    ...
    repfile.write("\nReport Date/Time    : " + str(datime.datime.now()))
